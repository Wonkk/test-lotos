...
$(document).ready(function(){
   ...
   if ($(window).width() < 960){
      $('.block-lotos-desctiprion').on('click', function(){
         if ($(this).find('.title-block-ddlotos').css('display') === 'none'){
            $(this).find('.c1-line').hide();
            $(this).find('.title-block-ddlotos').slideDown(1000);
         } else {
            $(this).find('.title-block-ddlotos').slideUp(1000);
            let x = $(this).find('.c1-line')
            setTimeout(function(){x.show();}, 1000);
         }
      })
